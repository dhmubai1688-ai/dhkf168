    ...
    async def _render_cleanup(self) -> float:
        ...
        try:
            ...
            if memory_mb > self.render_memory_limit:
                ...
                old_cache_size = stats.get("memory_size", 0)
                redis_enabled = global_cache.use_redis

                # ✅ 修复1：使用异步方法清理所有缓存（包括Redis）
                await global_cache.aclear_all()
                ...
                logger.info(
                    f"🆘 紧急清理完成:\n"
                    f"   ├─ 内存缓存: 清空 {old_cache_size} 项\n"
                    f"   ├─ GC 回收: {collected} 个对象\n"
                    f"   ├─ Redis缓存: {'✅ 已清理' if redis_enabled else '❌ 未启用'}\n"
