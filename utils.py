...
async def send_overtime_notification_async_with_data(notification_data: dict):
    ...
    try:
        ...
        if "bot" in globals():
            ...
            bot_sources.append("global")

        # 方法2：notification_service
        if not current_bot and notification_service and notification_service.bot:
            current_bot = notification_service.bot
            bot_sources.append("notification_service")

        # 方法3：bot_manager
        if not current_bot and bot_manager and hasattr(bot_manager, "bot"):
            current_bot = bot_manager.bot
            bot_sources.append("bot_manager")

        logger.info(
            f"🔍 通知发送 - bot来源: {bot_sources}, bot有效: {current_bot is not None}"
        )
