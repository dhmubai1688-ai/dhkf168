...
class Config:
    ...
    REDIS_URL = os.getenv("REDIS_URL", "")  # 优先使用完整URL
    REDIS_KEY_PREFIX = os.getenv("REDIS_KEY_PREFIX", "bot:")
    REDIS_DEFAULT_TTL = int(os.getenv("REDIS_DEFAULT_TTL", "300"))  # 5分钟
    REDIS_CACHE_ACTIVITY = os.getenv("REDIS_CACHE_ACTIVITY", "true").lower() == "true"
    REDIS_CACHE_USER = os.getenv("REDIS_CACHE_USER", "true").lower() == "true"
    REDIS_CACHE_GROUP = os.getenv("REDIS_CACHE_GROUP", "true").lower() == "true"

    # Redis降级配置
    REDIS_FALLBACK_TO_MEMORY = (
        os.getenv("REDIS_FALLBACK_TO_MEMORY", "true").lower() == "true"
    )
    REDIS_RECONNECT_ATTEMPTS = int(os.getenv("REDIS_RECONNECT_ATTEMPTS", "5"))
    REDIS_RECONNECT_DELAY = int(os.getenv("REDIS_RECONNECT_DELAY", "2"))
