...
class PostgreSQLDatabase:
    ...
    async def invalidate_cache(self, *keys: str):
        """使缓存失效"""
        await global_cache.adelete(*keys)

        if keys:
            logger.debug(f"缓存已失效: {keys}")
    ...
    async def force_refresh_activity_cache(self):
        ...
        await self.invalidate_cache(*cache_keys)
    ...
    async def get_group(self, chat_id: int) -> Optional[Dict]:
        ...
        async with self.pool.acquire() as conn:
            ...
            if row:
                result = dict(row)
                await global_cache.aset(cache_key, result, 300)
                return result
            return None
    ...
    async def get_user_cached(self, chat_id: int, user_id: int) -> Optional[Dict]:
        ...
        if row:
            ...
            await global_cache.aset(cache_key, result, 30)  # 30秒缓存
            return result
        return None
    ...
    async def get_activity_limits(self) -> Dict:
        ...
        cached = await global_cache.aget(cache_key)
        if cached is not None:
            logger.debug(f"从缓存获取活动配置: {len(cached)} 个活动")
            return cached
        ...
        try:
            ...
            # 存入缓存
            await global_cache.aset(cache_key, limits, 600)
            logger.debug(f"活动配置已存入缓存: {len(limits)} 个活动")
    ...
    async def get_fine_rates(self) -> Dict:
        ...
        async with self.pool.acquire() as conn:
            ...
            # 存入缓存
            await global_cache.aset(cache_key, fines, 600)
            return fines
    ...
    async def get_push_settings(self) -> Dict:
        ...
        async with self.pool.acquire() as conn:
            ...
            # 存入缓存
            await global_cache.aset(cache_key, settings, 300)
            return settings
