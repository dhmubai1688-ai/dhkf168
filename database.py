...
async def check_activity_limit(chat_id: int, uid: int, act: str):
    ...
    # 🆕 使用周期计数而不是今日计数
    current_count = await db.get_user_activity_count_for_period(chat_id, uid, act)
...
async def show_history(message: types.Message):
    ...
    async with OptimizedUserContext(chat_id, uid) as user:
        ...
        reset_hour = group_data.get("reset_hour", Config.DAILY_RESET_HOUR)
        reset_minute = group_data.get("reset_minute", Config.DAILY_RESET_MINUTE)
        period_str = f"{period_date} {reset_hour:02d}:{reset_minute:02d}"
        ...
        # 🆕 使用周期活动数据
        user_activities = await db.get_user_activities_for_period(chat_id, uid)
...
async def show_rank(message: types.Message):
    ...
    rank_text = f"🏆 本周期活动排行榜（{period_str}起）\n\n"
