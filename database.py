...
class PostgreSQLDatabase:
    ...
    async def complete_user_activity(
        ...
        logger.info(
            f"🔍 [数据库操作开始] 用户{user_id} 活动{activity} 时长{elapsed_time}s 周期{period_date}"
        )
        ...
        async with self.pool.acquire() as conn:
            async with conn.transaction():
                ...
                params = [elapsed_time, period_date]  # 🆕 使用周期日期
        ...
        logger.info(
            f"🔍 [数据库操作完成] 用户{user_id} 活动{activity} 完成更新，周期{period_date}"
        )
    ...
    async def get_current_period_date(self, chat_id: int) -> date:
        ...
        return current_period_start.date()

    async def get_user_activity_count_for_period(
        self, chat_id: int, user_id: int, activity: str
    ) -> int:
        """获取用户在当前周期的活动次数"""
        period_date = await self.get_current_period_date(chat_id)
        ...
        async with self.pool.acquire() as conn:
            ...
            count = row["activity_count"] if row else 0
            logger.debug(
                f"📊 获取周期活动计数: 用户{user_id} 活动{activity} 周期{period_date} 计数{count}"
            )
            return count

    async def get_user_activities_for_period(
        self, chat_id: int, user_id: int
    ) -> Dict[str, Dict]:
        """获取用户在当前周期的所有活动数据"""
        period_date = await self.get_current_period_date(chat_id)
