...
async def cmd_setdualmode(message: types.Message):
    ...
    try:
        if mode == "on":
            ...
            # ✅ 用您简洁的条件表达式
            cleanup_msg = (
                f"\n\n🔄 已清除 {deleted_count} 个旧的班次状态"
                if deleted_count > 0
                else ""
            )

            await message.answer(
                base_msg + cleanup_msg,  # ✅ 直接拼接，清晰明了
                parse_mode="HTML",
                reply_to_message_id=message.message_id,
            )
...
@admin_required
@rate_limit(rate=2, per=60)
async def cmd_redis_stats(message: types.Message):
    """查看Redis缓存统计"""
    from redis_cache import redis_cache_adapter

    stats = await redis_cache_adapter.get_stats()

    # 构建状态文本
    status_emoji = "✅" if stats.get("available") else "❌"

    text = (
        f"📊 <b>Redis缓存统计</b>\n\n"
        f"🔴 Redis状态: {status_emoji} {'可用' if stats.get('available') else '不可用'}\n"
        f"📈 Redis命中率: <code>{stats.get('redis_hit_rate', 0)*100:.2f}%</code>\n"
        f"🎯 Redis命中: <code>{stats.get('redis_hits', 0)}</code>\n"
        f"❌ Redis未命中: <code>{stats.get('redis_misses', 0)}</code>\n"
        f"⚠️ Redis错误: <code>{stats.get('redis_errors', 0)}</code>\n\n"
        f"💾 内存缓存大小: <code>{stats.get('memory_cache_size', 0)}</code> 项\n"
        f"📊 内存命中率: <code>{stats.get('memory_hit_rate', 0)*100:.2f}%</code>\n"
        f"🎯 内存命中: <code>{stats.get('memory_hits', 0)}</code>\n"
        f"❌ 内存未命中: <code>{stats.get('memory_misses', 0)}</code>\n"
    )

    if stats.get("redis_version"):
        text += (
            f"\n🔄 Redis版本: <code>{stats['redis_version']}</code>\n"
            f"🧠 内存使用: <code>{stats.get('redis_memory', 'N/A')}</code>\n"
            f"👥 客户端连接: <code>{stats.get('redis_clients', 0)}</code>\n"
            f"📝 总命令数: <code>{stats.get('total_commands_processed', 0)}</code>\n"
        )

    if not stats.get("available") and Config.REDIS_FALLBACK_TO_MEMORY:
        text += f"\n⚠️ <i>Redis不可用，当前使用内存缓存降级运行</i>"
...
@admin_required
@rate_limit(rate=1, per=60)
async def cmd_redis_clear(message: types.Message):
    """清理Redis缓存"""
    from redis_cache import redis_cache_adapter
    ...
    if len(args) > 1:
        pattern = args[1]
        await message.answer(
            f"⏳ 正在清理模式为 <code>{pattern}</code> 的缓存...", parse_mode="HTML"
        )
    else:
        await message.answer("⏳ 正在清理所有Redis缓存...")
    ...
    try:
        if pattern:
            ...
            keys = await redis_manager.client.keys(full_pattern)
            if keys:
                deleted = await redis_manager.client.delete(*keys)
                await message.answer(
                    f"✅ 已清理 {deleted} 个匹配 <code>{pattern}</code> 的缓存项",
                    parse_mode="HTML",
                )
            else:
                await message.answer(
                    f"ℹ️ 没有找到匹配 <code>{pattern}</code> 的缓存项", parse_mode="HTML"
                )
        else:
            # 清理所有
            await redis_cache_adapter.clear_all()
            await message.answer("✅ 所有Redis缓存已清空")

    except Exception as e:
        await message.answer(f"❌ 清理失败: {e}")
...
async def daily_reset_task():
    ...
    async def check_reset_idempotency(
        ...
        # ✅ 使用原子操作：检查并设置一次性完成
        is_first_execution = await global_cache.setnx(reset_flag_key, True, ttl=86400)

        if not is_first_execution:
            logger.debug(f"⏭️ 群组 {chat_id} {mode}模式 今天已执行重置")
            return False

        # 检查数据库（双重保障）- 只有当缓存是第一次时才检查数据库
        try:
            async with db.pool.acquire() as conn:
                ...
                if exists:
                    ...
                    # 数据库已有记录，但缓存没有（可能是新实例），删除缓存标记
                    await global_cache.adelete(reset_flag_key)
                    return False
    ...
    async def set_reset_idempotency(chat_id: int, mode: str, target_date: date):
        """设置幂等性标记 - 简化版（实际上不需要了，因为 check 函数已经设置）"""
        # 这个函数可以简化，因为 check_reset_idempotency 已经设置了标记
        # 但为了兼容性，保留一个简单版本
        reset_flag_key = f"{mode}_reset:{chat_id}:{target_date.strftime('%Y%m%d')}"

        # 确保标记存在
        await global_cache.aset(reset_flag_key, True, ttl=86400)
...
async def initialize_services():
    ...
    try:
        ...
        try:
            await redis_cache_adapter.initialize()
            logger.info("✅ Redis缓存适配器初始化完成")

            # 检查Redis是否可用
            if Config.REDIS_ENABLED:
                redis_stats = await redis_cache_adapter.get_stats()
                if redis_stats.get("available"):
                    logger.info(
                        f"📊 Redis连接状态: 可用, 版本: {redis_stats.get('redis_version', 'unknown')}"
                    )
                else:
                    logger.warning("⚠️ Redis连接失败，将使用内存缓存降级运行")
        except Exception as e:
            logger.error(f"❌ Redis初始化失败: {e}")
            if Config.REDIS_FALLBACK_TO_MEMORY:
                logger.warning("⚠️ Redis初始化失败，将使用内存缓存降级运行")
            else:
                raise
        ...
        # ========== 新增：14. 启动Redis监控任务 ==========
        if Config.REDIS_ENABLED:
            asyncio.create_task(redis_monitoring_task(), name="redis_monitoring")
            logger.info("✅ Redis监控任务已启动")

        # ========== 新增：15. 预热常用缓存 ==========
        if (
            Config.REDIS_CACHE_ACTIVITY
            or Config.REDIS_CACHE_USER
            or Config.REDIS_CACHE_GROUP
        ):
            asyncio.create_task(warm_up_cache(), name="cache_warmup")
        ...
        logger.info(
            f"📅 服务配置信息:\n"
            f"   ├─ Redis: {'✅ 启用' if Config.REDIS_ENABLED else '❌ 禁用'}\n"
            f"   ├─ Redis主机: {Config.REDIS_HOST}\n"
            f"   ├─ Redis端口: {Config.REDIS_PORT}\n"
            f"   ├─ Redis数据库: {Config.REDIS_DB}\n"
            f"   ├─ 缓存策略: 活动={Config.REDIS_CACHE_ACTIVITY}, 用户={Config.REDIS_CACHE_USER}, 群组={Config.REDIS_CACHE_GROUP}\n"
            f"   ├─ 降级策略: {Config.REDIS_FALLBACK_TO_MEMORY}\n"
        ...
        logger.error(f"调试信息 - Redis配置: REDIS_ENABLED={Config.REDIS_ENABLED}")
        raise


# ========== 新增：Redis监控任务 ==========
async def redis_monitoring_task():
    """Redis监控任务 - 定期检查Redis连接状态并记录统计信息"""
    logger.info("🚀 Redis监控任务已启动")
    ...
    while True:
        try:
            ...
            logger.info(
                f"📊 Redis缓存统计:\n"
                f"   ├─ 可用: {stats.get('available', False)}\n"
                f"   ├─ Redis命中率: {stats.get('redis_hit_rate', 0)*100:.1f}%\n"
                f"   ├─ Redis命中: {stats.get('redis_hits', 0)}\n"
                f"   ├─ Redis未命中: {stats.get('redis_misses', 0)}\n"
                f"   ├─ Redis错误: {stats.get('redis_errors', 0)}\n"
                f"   ├─ 内存缓存大小: {stats.get('memory_cache_size', 0)} 项\n"
                f"   └─ 内存命中率: {stats.get('memory_hit_rate', 0)*100:.1f}%"
            )

            # 如果Redis不可用但配置了降级，尝试重新连接
            if (
                not stats.get("available")
                and Config.REDIS_ENABLED
                and Config.REDIS_FALLBACK_TO_MEMORY
            ):
                logger.warning("⚠️ Redis连接丢失，尝试重新连接...")
                from redis_cache import redis_cache_adapter
...
async def warm_up_cache():
    ...
    try:
        ...
        # 1. 预热活动配置
        if Config.REDIS_CACHE_ACTIVITY:
            logger.info("🔄 预热活动配置缓存...")
            activity_limits = await db.get_activity_limits()
            if activity_limits:
                await redis_cache_adapter.set("activity_limits", activity_limits, 600)
                logger.info(f"✅ 活动配置缓存预热完成: {len(activity_limits)} 个活动")

        # 2. 预热群组配置（只预热有数据的群组）
        if Config.REDIS_CACHE_GROUP:
            logger.info("🔄 预热群组配置缓存...")
            all_groups = await db.get_all_groups()
            warmed = 0

            for chat_id in all_groups[:50]:  # 限制预热数量，避免压力过大
                try:
                    group_data = await db.get_group_cached(chat_id)
                    if group_data:
                        cache_key = f"group:{chat_id}"
                        await redis_cache_adapter.set(cache_key, group_data, 300)
                        warmed += 1
                except Exception as e:
                    logger.debug(f"预热群组 {chat_id} 失败: {e}")

                await asyncio.sleep(0.1)  # 避免请求过快

            logger.info(f"✅ 群组配置缓存预热完成: {warmed} 个群组")

        # 3. 预热罚款配置
        logger.info("🔄 预热罚款配置缓存...")
        fine_rates = await db.get_fine_rates()
        if fine_rates:
            await redis_cache_adapter.set("fine_rates", fine_rates, 600)
            logger.info(f"✅ 罚款配置缓存预热完成: {len(fine_rates)} 个活动")

        # 4. 预热上下班罚款配置
        work_fine_rates = await db.get_work_fine_rates()
        if work_fine_rates:
            await redis_cache_adapter.set("work_fine_rates", work_fine_rates, 600)
            logger.info(f"✅ 上下班罚款配置缓存预热完成: {len(work_fine_rates)} 个类型")
...
# ========== 修改 check_services_health 函数，添加Redis健康检查 ==========
async def check_services_health():
    ...
    health_status = {
        ...
        "redis": redis_stats.get("available", False) if Config.REDIS_ENABLED else True,
    ...
    # 记录详细的健康状态
    healthy_services = [k for k, v in health_status.items() if v and k != "timestamp"]
    unhealthy_services = [
        k
        for k, v in health_status.items()
        if not v and k not in ["timestamp", "redis_available"]
    ]

    # Redis特定警告
    if Config.REDIS_ENABLED and not health_status["redis"]:
        unhealthy_services.append("redis")
        logger.warning(f"⚠️ Redis服务不可用，正在使用内存缓存降级运行")
...
async def register_handlers():
    ...
    # ========== 新增：Redis相关命令 ==========
    dp.message.register(cmd_redis_stats, Command("redisstats"))
    dp.message.register(cmd_redis_clear, Command("redisclear"))
